...
/// <summary>
/// The OldInputSystem takes player input, and manipulates the game in response.
/// </summary>
public class OldInputSystem
{
	...
	public void PollForInput(GameRoot game, GameTime gameTime)
	{
		...
		{
			var clickedEntity = ResolveMousePositionToEntity(game, game.OldWorld, Mouse.GetState());
			if (clickedEntity != OldWorld.NullEntityID)
			{
				game.OldWorld.AppearanceComponents.Get(clickedEntity).SpriteColor = Color.Red;
			}
		}
		...
		if (Mouse.GetState().MiddleButton == ButtonState.Pressed)
		{
			...
			var cameraPos = game.OldWorld.PositionComponents.Get(game.OldWorld.CameraEntity).Position;
			var newPos = new Vector2(
				cameraPos.X - (mousePos.X - _fixedTogglePoint.X),
				cameraPos.Y - (mousePos.Y - _fixedTogglePoint.Y)
			);
			game.OldCameraSystem.SetCamera(game.OldWorld, newPos);
		...
		else
		{
			...
			{
				game.OldCameraSystem.MoveCamera(game.OldWorld, movementDirection, cameraSpeed, gameTime);
				game.OldWorld.PositionComponents.Get(game.OldWorld.CameraEntity).Position += movementDirection * cameraSpeed *
		...
		{
			game.OldRenderingSystem.DrawBoundingBoxes = !game.OldRenderingSystem.DrawBoundingBoxes;
		}
	...
	/// <param name="oldWorld">The <see cref="OldWorld"/> that holds all mutable state for the game (or a particular unit test).</param>
	/// <param name="state"><see cref="MouseState"/> of the player's mouse, generally obtained with <see cref="Mouse.GetState()"/>.</param>
	/// <returns>Entity number for what the player has clicked on, or <see cref="OldWorld.NullEntityID"/> if no
	/// entities could be resolved.</returns>
	public int ResolveMousePositionToEntity(GameRoot game, OldWorld oldWorld, MouseState state)
	{
		...
		var mouseWorldPosition = mousePosition + oldWorld.PositionComponents.Get(oldWorld.CameraEntity).Position;
		...
		var topLeftCorner = oldWorld.Grid.VectorToTileCoordinate(topLeftPosition);
		var bottomRightCorner = oldWorld.Grid.VectorToTileCoordinate(bottomRightPosition);
		...
		for (var x = topLeftCorner.xCoordinate; x < bottomRightCorner.xCoordinate; x++)
		{
			for (var y = topLeftCorner.yCoordinate; y < bottomRightCorner.yCoordinate; y++)
			{
				...
				{
					var entity = oldWorld.Grid.Grid[x, y, layer];
					if (entity == OldWorld.NullEntityID)
					{
						continue;
					}

					//var boundingBox = oldWorld.AppearanceComponents.Get(entity).SpriteTexture.Bounds;
					var boundingBox = game.TextureSystem.Textures[oldWorld.AppearanceComponents.Get(entity).TextureName].Bounds;
					boundingBox.Offset(oldWorld.PositionComponents.Get(entity).Position);
		...
		{
			//var texture = oldWorld.AppearanceComponents.Get(entity).SpriteTexture;
			var texture = game.TextureSystem.Textures[oldWorld.AppearanceComponents.Get(entity).TextureName];
			var rawData = new Color[1];

			var localClickCoordinates = mouseWorldPosition + -oldWorld.PositionComponents.Get(entity).Position;
		...
		{
			var entityPosition = oldWorld.PositionComponents.Get(entity).Position;
		...
		var winner = OldWorld.NullEntityID;
